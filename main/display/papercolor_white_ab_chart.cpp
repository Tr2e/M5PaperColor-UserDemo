...
#if defined(CONFIG_PAPERCOLOR_WHITE_AB_CHART_ON_BOOT) && CONFIG_PAPERCOLOR_WHITE_AB_CHART_ON_BOOT
#include <M5Unified.h>
#include <array>
#include "display/display_metrics.h"
#include "esp_log.h"

extern const uint8_t white_ab_start[] asm("_binary_white_ab_v1_bin_start");
extern const uint8_t white_ab_end[] asm("_binary_white_ab_v1_bin_end");

bool papercolor_show_white_ab_chart()
{
    using namespace papercolor_white_ab_chart;
    if (M5.Display.width()!=600 || M5.Display.height()!=400) return false;
    const size_t size=static_cast<size_t>(white_ab_end-white_ab_start);
    if (!valid_payload(white_ab_start,size)) {
        ESP_LOGE("WhiteAB","Invalid pre-rendered payload");
        return false;
    }
    DisplayMetricsTrace metrics("white_ab_chart");
    ...
    labels.drawString("WHITE A/B PHOTO",12,8);
    labels.setTextSize(1);
    labels.drawString("v1  A: ORIGINAL WHITE  B: REDUCED WHITE",12,30);
    ...
    for (int y=0;y<400;++y) {
        for (int x=0;x<600;++x) {
            ...
            sample(lx,ly,white_ab_start,size,native);
    ...
    metrics.finish(true);
    ESP_LOGI("WhiteAB","v1: source-chart PhotoBalanced crops; A white-off / B white-on; hue and edge guard fixed");
    return true;
}
#else
bool papercolor_show_white_ab_chart() { return false; }
#endif
