...
#if defined(CONFIG_PAPERCOLOR_WARM_AB_CHART_ON_BOOT) && CONFIG_PAPERCOLOR_WARM_AB_CHART_ON_BOOT
#include <M5Unified.h>
#include <array>
#include "display/display_metrics.h"
#include "esp_log.h"

extern const uint8_t warm_ab_start[] asm("_binary_warm_ab_v1_bin_start");
extern const uint8_t warm_ab_end[] asm("_binary_warm_ab_v1_bin_end");
...
bool papercolor_show_warm_ab_chart()
{
    ...
    const size_t size=static_cast<size_t>(warm_ab_end-warm_ab_start);
    if (!valid_payload(warm_ab_start,size)) {
        ESP_LOGE("WarmAB","Invalid pre-rendered payload");
        return false;
    }
    DisplayMetricsTrace metrics("warm_ab_chart");
    ...
    labels.drawString("WARM A/B PHOTO",12,8);
    labels.setTextSize(1);
    labels.drawString("v1  A: OLD COMP  B: BYPASS",12,30);
    ...
    for (int y=0;y<400;++y) {
        for (int x=0;x<600;++x) {
            ...
            sample(lx,ly,warm_ab_start,size,native);
    ...
    metrics.finish(true);
    ESP_LOGI("WarmAB","v1: source-chart PhotoBalanced crops; A old warm compensation / B bypass; white reduction, hue and edge guard fixed");
    return true;
}
#else
bool papercolor_show_warm_ab_chart() { return false; }
#endif
