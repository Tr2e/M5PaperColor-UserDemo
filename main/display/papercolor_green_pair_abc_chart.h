...
namespace papercolor_green_pair_abc_chart {
constexpr int WIDTH=400,HEIGHT=600,PATCH_WIDTH=88,PATCH_HEIGHT=50,ROW_COUNT=6;
...
constexpr const char* LABELS[]={"11 LIME  B:+GREEN C:-GREEN","12 TEAL RATIO B:-GREEN C:+GREEN",
 "12 TEAL WHITE B:HALF C:NONE","10 BLUE CYAN EDGE CONTROL",
 "20 CYAN CONTROL","17 LIGHT PURPLE CONTROL"};
inline bool valid_payload(const uint8_t* data,size_t size){
 if(!data||size!=PAYLOAD_BYTES)return false;
 for(size_t i=0;i<size;++i)for(uint8_t c:{uint8_t(data[i]>>4),uint8_t(data[i]&15)})if(c>6||c==4)return false;
 return true;
}
inline bool sample(int x,int y,const uint8_t* data,size_t size,uint8_t& native){
 ...
 for(int i=0;i<6;++i)if(x>=12+i*64&&x<68+i*64&&y>=52&&y<72){native=papercolor_native_chart::SOLIDS[i];return true;}
 if(y<ROW_Y||x<COLUMN_X)return false;
 const int row=(y-ROW_Y)/ROW_STEP,py=(y-ROW_Y)%ROW_STEP,col=(x-COLUMN_X)/COLUMN_STEP,px=(x-COLUMN_X)%COLUMN_STEP;
