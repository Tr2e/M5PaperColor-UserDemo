...
#if defined(CONFIG_PAPERCOLOR_GREEN_PAIR_ABC_CHART_ON_BOOT) && \
    CONFIG_PAPERCOLOR_GREEN_PAIR_ABC_CHART_ON_BOOT
...
extern const uint8_t green_pair_start[] asm("_binary_green_pair_abc_v1_bin_start");
extern const uint8_t green_pair_end[] asm("_binary_green_pair_abc_v1_bin_end");

bool papercolor_show_green_pair_abc_chart()
{
    using namespace papercolor_green_pair_abc_chart;

    if (M5.Display.width() != 600 || M5.Display.height() != 400) {
        return false;
    }

    const size_t size = green_pair_end - green_pair_start;
    if (!valid_payload(green_pair_start, size)) {
        return false;
    }

    DisplayMetricsTrace metrics("green_pair_abc_chart");
    ...
    labels.drawString("GREEN PAIR A/B/C", 12, 8);
    labels.setTextSize(1);
    labels.drawString("A: NOW   B/C: ONE AXIS ONLY", 12, 30);
    ...
    labels.drawString("A B C A; ratio and white tested separately", 12, 552);
    ...
    for (int y = 0; y < 400; ++y) {
        for (int x = 0; x < 600; ++x) {
            ...
            sample(logical_x, logical_y, green_pair_start, size, native);
