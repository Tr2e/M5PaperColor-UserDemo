...
namespace papercolor_purple_white_abc_chart {
...
constexpr const char* LABELS[] = {
    "09 PURPLE WHITE B:HALF C:NONE",
    "18 DARK PURPLE CONTROL",
    "17 LIGHT PURPLE CONTROL",
    "16 MAGENTA CONTROL",
    "12 TEAL CONTROL",
...
{
    if (!data || size != PAYLOAD_BYTES || x < 0 || y < 0 || x >= WIDTH || y >= HEIGHT) {
        return false;
    }
    ...
    if (row >= ROW_COUNT || patch_y >= PATCH_HEIGHT || column >= 4 || patch_x >= PATCH_WIDTH) {
        return false;
    }
