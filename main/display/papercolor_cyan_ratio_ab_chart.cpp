...
#if defined(CONFIG_PAPERCOLOR_CYAN_RATIO_AB_CHART_ON_BOOT) && CONFIG_PAPERCOLOR_CYAN_RATIO_AB_CHART_ON_BOOT
#include <M5Unified.h>
#include <array>
#include "display/display_metrics.h"
#include "esp_log.h"

extern const uint8_t cyan_ratio_ab_start[] asm("_binary_cyan_ratio_ab_v1_bin_start");
extern const uint8_t cyan_ratio_ab_end[] asm("_binary_cyan_ratio_ab_v1_bin_end");

bool papercolor_show_cyan_ratio_ab_chart()
{
    using namespace papercolor_cyan_ratio_ab_chart;
    if (M5.Display.width()!=600 || M5.Display.height()!=400) return false;
    const size_t size=static_cast<size_t>(cyan_ratio_ab_end-cyan_ratio_ab_start);
    if (!valid_payload(cyan_ratio_ab_start,size)) {
        ESP_LOGE("CyanRatioAB","Invalid pre-rendered payload");
        return false;
    }
    DisplayMetricsTrace metrics("cyan_ratio_ab_chart");
    ...
    labels.drawString("CYAN RATIO SMOOTH A/B",12,8);
    labels.setTextSize(1);
    labels.drawString("v1  A: NOW  B: LESS GREEN",12,30);
    ...
    for (int y=0;y<400;++y) {
        for (int x=0;x<600;++x) {
            ...
            sample(lx,ly,cyan_ratio_ab_start,size,native);
    ...
    metrics.finish(true);
    ESP_LOGI("CyanRatioAB","v1: PhotoBalanced native crops; A baseline / B smooth cyan ratio; cyan patch, two ramps and three canonical controls");
    return true;
}
#else
bool papercolor_show_cyan_ratio_ab_chart() { return false; }
#endif
