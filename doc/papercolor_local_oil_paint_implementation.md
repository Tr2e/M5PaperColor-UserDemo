...
EzData 绑定二维码（包括生成失败提示页）也属于非照片画面，在其绘制入口统一失效照片登记并释放快照，防止 A 键误处理二维码。
...
A 按下时在 Canvas claim 内捕获照片 generation，确认单击时在同一互斥机制内校验。等待期间换图、二维码覆盖或退出页面后，旧操作失效；长按、多击和忙碌期间的操作也会清除待处理目标。
...
算法在降采样、梯度、底层和笔触阶段提供调度检查点；控制器每累计约 20ms 计算时间才阻塞一个 tick。当前 100Hz 配置下一个 tick 为 10ms，避免 `pdMS_TO_TICKS(1)` 截断为零，也不逐行固定增加等待。
...
控制器测试使用模拟 Canvas/PSRAM 与真实 M5Unified 按键识别器，直接编译生产控制器，覆盖单击确认期间换图、非照片失效、原图恢复、两次大缓冲分配分别失败及 Canvas 忙碌时的无副作用拒绝。
