          ...
          reference(aw * ah), detail(PAINT_DETAIL ? w * h : 0), output(w * h)
    {
        ...
        for (int y = 0; y < height; ++y) for (int x = 0; x < width; ++x) {
            ...
            if (PAINT_BACKGROUND_CONTROL) {
                ...
                // Interpolate broad ground toward a LOW-PASS guide, not the
                // fine reference: that would reintroduce RGB565 steps and fog
                // texture into precisely the regions meant to stay quiet.
                if (q > 0) {
                    const int reach = scaled(9);
                    Color sum{};
                    for (int dy = -1; dy <= 1; ++dy) for (int dx = -1; dx <= 1; ++dx) {
                        const Color c = sample(x + dx * reach, y + dy * reach);
                        sum.r += c.r; sum.g += c.g; sum.b += c.b;
                    }
                    ground_color = mix(ground_color, {sum.r / 9, sum.g / 9, sum.b / 9}, 0.80f * q * q);
                }
            }
