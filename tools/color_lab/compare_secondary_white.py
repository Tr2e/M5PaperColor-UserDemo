#!/usr/bin/env python3
"""Full-chart/continuous-target A/B against the IMG_9934 renderer.
...
FLAGS = ['-DCONFIG_PAPERCOLOR_PRIMARY_WHITE_BUDGET=1',
         ...
         '-DCONFIG_PAPERCOLOR_BLUE_SECONDARY_BALANCE=1']
...
def main():
    ...
    baseline_hash=json.loads((ROOT/'artifacts/color_calibration/secondary-balance-comparison.json').read_text())['variants']['candidate']['frame_sha256']
    pixels=list(chart.getdata())
    report={'baseline':'IMG_9934 secondary balance', 'candidate':'gentle secondary white reduction',
        ...
        for enabled in (False,True):
            ...
            def build(probe):
                ...
                subprocess.run(['c++','-std=c++17','-O2','-Wall','-Wextra','-Werror',*FLAGS,
                    '-DCONFIG_PAPERCOLOR_SECONDARY_WHITE_REDUCTION='+str(int(enabled)),
                    '-I'+str(ROOT/'main'),
            ...
            for p in spec['patches']:
                ...
                if p['id'] == 10:
                    result['patches'][-1]['rare_code_coordinates'] = {NAMES[c]: [[i%400,i//400] for i in indices if frame[i]==c] for c in (0,1)}
                if p['id'] in (5,7,8,19):
            ...
            controls={'red':(255,0,0),'green':(0,255,0),'blue':(0,0,255),'native_blue':(100,64,255),
                      ...
                      'pale_magenta':(240,224,240),'pale_cyan':(224,240,240)}
    ...
    for key in ('red','green','blue','native_blue','gray96','pale_red','orange','lime','pale_magenta','pale_cyan'):
    ...
    for key in ('magenta','cyan','light_purple','blue_cyan'):
        a,b=(v['uniform'][key]['native_percent'] for v in (base,candidate))
        assert abs(a['black']-b['black'])<.3,key
        assert b['white']<=a['white']+.2,key
        assert b['white']>=a['white']*.75-.2,key
        pigment='red' if key in ('magenta','light_purple') else 'green'
        assert abs(a[pigment]/(a[pigment]+a['blue'])-b[pigment]/(b[pigment]+b['blue']))<.003,key
    for key,lo,hi in [('magenta',13,15),('cyan',22,24)]:
        assert lo<candidate['uniform'][key]['native_percent']['white']<hi
    ...
    assert old.shape == new.shape == (65536,3)
    ...
    for mask,pigment in ((43,[191,0,0]),(99,[67,138,28])):
        ...
        assert black_error<=2*black_bound
        assert np.all(b[:,0]<=a[:,0]+2*bounds[0])
        assert np.all(b[:,0]>=a[:,0]*.75-1.75*bounds[0])
        cross_error=np.abs(a[:,1]*b[:,2]-a[:,2]*b[:,1])
        ratio_bound=bounds[1]*(np.abs(a[:,2])+np.abs(b[:,2]))+bounds[2]*(np.abs(a[:,1])+np.abs(b[:,1]))+2*bounds[1]*bounds[2]
        assert np.all(cross_error<=ratio_bound+1e-6)
        max_ratio_cross_error=max(max_ratio_cross_error,float(max(cross_error)))
    ...
    continuity={'edge_count':len(old_steps),'baseline_max_step':float(max(old_steps)),
        ...
        'max_black_weight_drift':max_black_error,
        'max_chromatic_ratio_cross_error':max_ratio_cross_error,
    ...
    # No larger worst-case seam than the prior candidate target map. Retain old seams
    ...
    dest=ROOT/'artifacts/color_calibration/secondary-white-comparison.json'
