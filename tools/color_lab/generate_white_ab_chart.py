#!/usr/bin/env python3
"""Pre-render white off/on with the real photo code, then compose native A/B/B/A crops.
...
INPUTS=['main/display/papercolor_photo_dither.cpp','main/display/papercolor_photo_dither.h',
        ...
        'main/display/papercolor_native_chart.h','main/display/papercolor_white_ab_chart.h',
        'tools/color_lab/probe_frame.cpp','tools/color_lab/probe_white_ab_chart.cpp',
        'tools/color_lab/generate_white_ab_chart.py','artifacts/color_lut/nominal-5bit.lut',
...
def main():
    ...
    reference=json.loads((ROOT/'artifacts/color_calibration/chromatic-edge-comparison.json').read_text())
    for name,h in reference['source_hashes'].items():
        assert sha((ROOT/name).read_bytes())==h, f'IMG_9938 source changed: {name}'
    frames={}; crops={}; records=[]; payload=bytearray()
    with tempfile.TemporaryDirectory(prefix='papercolor-white-ab-') as directory:
        tmp=Path(directory)
        for enabled,name in ((False,'A'),(True,'B')):
            ...
            assert render()==frame
            frames[name]=frame
        assert sha(frames['B'])==reference['variants']['candidate']['frame_sha256'], 'B must reproduce IMG_9938 exactly'
        ...
        packed=tmp/'white-ab-v1.bin';packed.write_bytes(payload)
        binary=tmp/'sampler'
        subprocess.run(['c++','-std=c++17','-O2','-Wall','-Wextra','-Werror',
            '-I'+str(ROOT/'main'),str(ROOT/'tools/color_lab/probe_white_ab_chart.cpp'),'-o',str(binary)],check=True)
    ...
    label('WHITE A/B PHOTO',12,8,True)
    label('v1  A: ORIGINAL WHITE  B: REDUCED WHITE',12,30)
    ...
    labels=['16 MAGENTA  255,0,255','17 LIGHT PURPLE  153,51,204','20 CYAN  0,255,255',
    ...
    dest=ROOT/'artifacts/color_calibration/white-ab-v1'
    ...
    manifest={'version':1,'warning':'Nominal preview only. Crops are real host-compiled PhotoBalanced outputs, transferred as native codes without a second dither. Physical texture/color still requires panel review.',
        ...
        'A':'secondary white reduction OFF; hue and edge guard ON',
        'B':'secondary white reduction ON; exact IMG_9938 full-frame output',
    ...
    hashes['artifacts/color_calibration/white-ab-v1.bin']=sha(payload)
    dest.with_suffix('.sha256').write_text(''.join(h+' '+p+'\n' for p,h in hashes.items()))
    print('Verified: B reproduces IMG_9938; all 24 crops and native anchors match C++ sampler.')
