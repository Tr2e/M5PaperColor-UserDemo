#!/usr/bin/env python3
"""Pre-render legacy/smooth purple compensation with the real photo code, then compose native A/B/B/A crops.
...
FLAGS=['PRIMARY_WHITE_BUDGET','PRIMARY_PEAK_PRESERVATION','EXACT_PIGMENT_ANCHOR',
       'BLUE_SECONDARY_BALANCE','CHROMATIC_EDGE_GUARD','SECONDARY_WHITE_REDUCTION','WARM_COMPENSATION_BYPASS']
...
INPUTS=['main/display/papercolor_photo_dither.cpp','main/display/papercolor_photo_dither.h',
        ...
        'main/display/papercolor_native_chart.h','main/display/papercolor_purple_smooth_ab_chart.h','main/display/papercolor_purple_smooth_ab_chart.cpp',
        'tools/color_lab/probe_frame.cpp','tools/color_lab/probe_purple_smooth_ab_chart.cpp',
        'tools/color_lab/generate_purple_smooth_ab_chart.py','artifacts/color_lut/nominal-5bit.lut',
...
def main():
    ...
    stimulus=chart.copy()
    ramp_specs={-1:{'rect':patches[13]['rect_xyxy'],'a':[70,50,200],'b':[140,50,200]},
                -2:{'rect':patches[14]['rect_xyxy'],'a':[100,50,160],'b':[100,50,255]}}
    ...
    with tempfile.TemporaryDirectory(prefix='papercolor-purple-smooth-ab-') as directory:
        tmp=Path(directory)
        for enabled,name in ((False,'A'),(True,'B')):
            binary=tmp/('render-'+name)
            subprocess.run(['c++','-std=c++17','-O2','-Wall','-Wextra','-Werror',
                *['-DCONFIG_PAPERCOLOR_'+f+'=1' for f in FLAGS],
                '-DCONFIG_PAPERCOLOR_PURPLE_COMPENSATION_SMOOTH='+str(int(enabled)),
        ...
        assert sha(frames['A'])==reference['variants']['baseline']['frame_sha256'], 'A must reproduce IMG_9949 exactly'
        assert sha(frames['B'])==reference['variants']['candidate']['frame_sha256'], 'B canonical chart must reproduce IMG_9949 exactly'
        ...
        for row,pid in enumerate(IDS):
            ...
            if pid>0:assert crops[row,'A']==crops[row,'B']
            else:assert crops[row,'A']!=crops[row,'B']
            records.append(record)
        assert len(payload)==26400
        packed=tmp/'purple-smooth-ab-v1.bin';packed.write_bytes(payload)
        binary=tmp/'sampler'
        subprocess.run(['c++','-std=c++17','-O2','-Wall','-Wextra','-Werror',
            '-I'+str(ROOT/'main'),str(ROOT/'tools/color_lab/probe_purple_smooth_ab_chart.cpp'),'-o',str(binary)],check=True)
    ...
    label('PURPLE TRANSITION A/B',12,8,True)
    label('v1  A: OLD STEP  B: SMOOTH',12,30)
    ...
    labels=['RAMP R:70-140  G:50 B:200','RAMP B:160-255 R:100 G:50',
    ...
    dest=ROOT/'artifacts/color_calibration/purple-smooth-ab-v1'
    ...
    manifest={'version':1,'warning':'Nominal preview only. Crops are real host-compiled PhotoBalanced outputs, transferred as native codes without a second dither. Physical texture/color still requires panel review.',
        ...
        'A':'legacy hard purple activation; canonical frame equals IMG_9949',
        'B':'smooth purple activation across R/B=2/5..5/8; canonical frame equals IMG_9949',
    ...
    hashes['artifacts/color_calibration/purple-smooth-ab-v1.bin']=sha(payload)
    dest.with_suffix('.sha256').write_text(''.join(h+' '+p+'\n' for p,h in hashes.items()))
    print('Verified: both canonical frames reproduce IMG_9949; separate ramps differ; all 24 crops and native anchors match C++ sampler.')
