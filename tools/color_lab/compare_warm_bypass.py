#!/usr/bin/env python3
"""Warm-compensation bypass A/B against IMG_9938; nominal, not physical QA."""
import csv
import hashlib
import io
import json
...
def main():
    ...
    old = json.loads((ROOT/'artifacts/color_calibration/chromatic-edge-comparison.json').read_text())
    ...
    report = {'baseline':'IMG_9938 chromatic edge','candidate':'warm source compensation bypass',
    ...
    frames, traces = {}, {}
    with tempfile.TemporaryDirectory(prefix='papercolor-warm-bypass-') as directory:
        tmp = Path(directory)
        ...
        for enabled in (False,True):
            ...
            flags += ['-DCONFIG_PAPERCOLOR_WARM_COMPENSATION_BYPASS='+str(int(enabled))]
    ...
    for key in controls:
        if key!='orange': assert base['uniform'][key]==new['uniform'][key],key
    ...
    eligible=(masks==15)&(rgb[:,2]>=32)&(rgb[:,0]>=rgb[:,1]+80)
    ...
    for a,b in zip(traces['baseline'],traces['candidate']):
        if int(b['mask'])==15:
            ...
            assert all(b['target_'+c]==b['uncompensated_'+c] for c in 'rgb')
    index=np.arange(65536); old_steps=[]; new_steps=[]; boundary=[]; warm_edges=[]
    for step,valid in ((2048,(index>>11)<31),(32,((index>>5)&63)<63),(1,(index&31)<31)):
        ...
        warm_edges.extend(eligible[a]|eligible[b])
    old_steps,new_steps=np.array(old_steps),np.array(new_steps)
    boundary=np.array(boundary); warm_edges=np.array(warm_edges)
    ...
    continuity={'changed_targets':int(sum(changed)),'eligible_targets':int(sum(eligible)),
        ...
        'warm_touching_edges':int(sum(warm_edges)),
        'old_warm_max':float(max(old_steps[warm_edges])),'new_warm_max':float(max(new_steps[warm_edges])),
    ...
    for key in ramps:
        ...
        if not key.startswith('orange_'): assert delta<4,key
        ...
        if not key.startswith('orange_'): assert envelope_delta<4,key
    ...
    dest = ROOT/'artifacts/color_calibration/warm-bypass-comparison.json'
