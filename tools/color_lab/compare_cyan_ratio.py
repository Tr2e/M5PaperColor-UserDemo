#!/usr/bin/env python3
"""General cyan ratio candidate against IMG_9952; nominal diagnostics, not physical QA."""
import csv
import hashlib
import io
import json
...
FLAGS = ['PRIMARY_WHITE_BUDGET','PRIMARY_PEAK_PRESERVATION','EXACT_PIGMENT_ANCHOR',
         'BLUE_SECONDARY_BALANCE','SECONDARY_WHITE_REDUCTION','CHROMATIC_EDGE_GUARD','WARM_COMPENSATION_BYPASS','PURPLE_COMPENSATION_SMOOTH']
...
def main():
    ...
    old = json.loads((ROOT/'artifacts/color_calibration/purple-smooth-comparison.json').read_text())
    ...
    ramps = {'purple_ratio_threshold':((70,50,200),(130,50,200)),
             ...
             'cyan_hue_green':((0,255,140),(0,255,255)),
             'cyan_hue_blue':((0,140,255),(0,255,255)),
             'cyan_chroma':((120,144,144),(120,255,255)),
             'cyan_tie':((32,170,210),(32,250,210)),
    ...
    report = {'baseline':'IMG_9952 purple smooth','candidate':'cyan ratio smooth coverage transfer',
    ...
    for p in ['main/display/papercolor_photo_dither.cpp','main/display/papercolor_photo_dither.h',
              'main/display/papercolor_gamut.h','main/display/papercolor_cyan_ratio.h','tools/color_lab/compare_cyan_ratio.py','tools/color_lab/probe_frame.cpp',
              'tools/color_lab/probe_targets.cpp','artifacts/color_lut/nominal-5bit.lut']:
        report['source_hashes'][p] = sha((ROOT/p).read_bytes())
    frames, traces = {}, {}
    with tempfile.TemporaryDirectory(prefix='papercolor-cyan-ratio-') as directory:
        tmp = Path(directory)
        ...
        for enabled in (False,True):
            ...
            flags += ['-DCONFIG_PAPERCOLOR_CYAN_RATIO_SMOOTH='+str(int(enabled))]
    ...
    protected_controls=set(controls)-{'cyan','pale_cyan'}
    for key in protected_controls:
        assert base['uniform'][key]==new['uniform'][key],key
    ...
    g=rgb[:,1]-rgb[:,0];b=rgb[:,2]-rgb[:,0]
    maximum=np.maximum(g,b);minimum=np.minimum(g,b)
    eligible=(masks==99)&(maximum>24)&(minimum>0.75*maximum)
    changed=np.any(previous!=current,axis=1)
    assert not np.any(changed & ~eligible)
    assert 500 < sum(changed) < 10000, int(sum(changed))
    # Reconstruct green/blue/white coverage independently from rounded Q4 RGB.
    matrix=np.array([[255,255,255],[67,138,28],[100,64,255]],float).T
    inverse=np.linalg.inv(matrix)
    old_weights=previous[changed]@inverse.T;new_weights=current[changed]@inverse.T
    white_bound=np.abs(inverse[0]).sum()/16+1e-6
    total_bound=np.abs(inverse.sum(axis=0)).sum()/16+1e-6
    assert np.max(np.abs(new_weights[:,0]-old_weights[:,0]))<=white_bound
    assert np.max(np.abs(new_weights.sum(1)-old_weights.sum(1)))<=total_bound
    assert new_weights[:,1:].min()>-0.001
    cyan_old=previous[0x07ff]@inverse.T;cyan_new=current[0x07ff]@inverse.T
    assert abs(cyan_new[1]/sum(cyan_new[1:])-cyan_old[1]/sum(cyan_old[1:])+0.0625)<0.001
    # Compare ALL targets in all canonical fills; downstream diffusion can
    # change phase elsewhere, so counts/crops are also recorded independently.
    for patch in spec['patches']:
        r,g,b=patch['rgb'];idx=(r>>3)<<11|(g>>2)<<5|(b>>3)
        if patch['id']!=20: assert np.array_equal(previous[idx],current[idx]),patch['id']
    old20,new20=(v['patches'][19]['native_counts'] for v in (base,new))
    assert abs(old20['white']-new20['white'])<=10
    assert old20['black']==new20['black']==0
    assert new20['green']<old20['green'] and new20['blue']>old20['blue']
    index=np.arange(65536); old_steps=[]; new_steps=[]; boundary=[]; cyan_edges=[]
    for step,valid in ((2048,(index>>11)<31),(32,((index>>5)&63)<63),(1,(index&31)<31)):
        ...
        boundary.extend((eligible[a]!=eligible[b]) & (masks[a]==masks[b]))
        cyan_edges.extend(eligible[a]|eligible[b])
    old_steps,new_steps=np.array(old_steps),np.array(new_steps)
    boundary=np.array(boundary); cyan_edges=np.array(cyan_edges)
    ...
    continuity={'changed_targets':int(sum(changed)),'eligible_targets':int(sum(eligible)),
        ...
        'cyan_touching_edges':int(sum(cyan_edges)),
        'old_cyan_max':float(max(old_steps[cyan_edges])),'new_cyan_max':float(max(new_steps[cyan_edges])),
        'old_activation_boundary_max':float(max(old_steps[boundary])),
        'new_activation_boundary_max':float(max(new_steps[boundary]))}
    assert continuity['new_max']<=continuity['old_max']+0.1
    PROTECTED_RAMPS={'purple_ratio_threshold','purple_blue_threshold','purple_light_dark','blue_black','blue_white','blue_magenta','red_white','neutral','magenta_white','magenta_black','edge_white','edge_black','edge_neutral_red','red_blue_edge',*[k for k in ramps if k.startswith('orange_')]}
    ...
    report['checks'] = {'targets_scanned':len(traces['candidate']),'continuity':continuity,
        'protected_uniform_byte_identical':len(protected_controls),
        ...
        'q4_white_max_change':float(np.max(np.abs(new_weights[:,0]-old_weights[:,0]))),
        'q4_white_bound':float(white_bound),
        'q4_total_max_change':float(np.max(np.abs(new_weights.sum(1)-old_weights.sum(1)))),
        'q4_total_bound':float(total_bound),
        'changed_chart_pixels':int(np.sum(frames['baseline']!=frames['candidate']))}
    dest = ROOT/'artifacts/color_calibration/cyan-ratio-comparison.json'
    for name,frame in frames.items():
        (ROOT/('artifacts/color_calibration/cyan-ratio-'+name+'.codes')).write_bytes(frame.tobytes())
