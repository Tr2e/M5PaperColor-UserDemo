#!/usr/bin/env python3
"""Compose patch-09 white-axis A/B/C crops from complete photo renders."""
...
ROWS = [(9, [0, 1, 2]), (18, [0, 0, 0]), (17, [0, 0, 0]),
        (16, [0, 0, 0]), (12, [0, 0, 0]), (10, [0, 0, 0])]
LABELS = ["09 PURPLE WHITE B:HALF C:NONE", "18 DARK PURPLE CONTROL",
          "17 LIGHT PURPLE CONTROL", "16 MAGENTA CONTROL", "12 TEAL CONTROL",
          "10 BLUE CYAN EDGE CONTROL"]
BASELINE_SHA256 = "7943d33235ac6f12fb74d762343af215adab36ff7cc928ad9f5e7e4422dbb7d6"
INPUTS = [
    "main/display/papercolor_photo_dither.cpp", "main/display/papercolor_photo_dither.h",
    "main/display/papercolor_gamut.h", "main/display/papercolor_cyan_ratio.h",
    "main/display/papercolor_green_pair_refinement.h", "main/display/papercolor_lut.cpp",
    "main/display/papercolor_purple_white_abc_chart.h",
    "main/display/papercolor_purple_white_abc_chart.cpp",
    "tools/color_lab/purple_white_probe.h",
    "tools/color_lab/generate_purple_white_abc_chart.py",
    "tools/color_lab/probe_frame.cpp", "tools/color_lab/probe_purple_white_abc_chart.cpp",
...
def main():
    ...
    modified = "#include \"purple_white_probe.h\"\n" + source.replace(
        needle,
        "        p = papercolor_diagnostic::purple_white_target("
        "original, p, PAPERCOLOR_DIAGNOSTIC_PURPLE_WHITE_MODE);\n" + needle)

    frames = {}
    crops = {}
    records = []
    payload = bytearray()
    with tempfile.TemporaryDirectory(prefix="papercolor-purple-white-") as directory:
        tmp = Path(directory)
        implementation = tmp / "diagnostic.cpp"
        ...
        for mode in range(3):
            ...
            subprocess.run([
                ...
                *["-DCONFIG_PAPERCOLOR_" + flag + "=1" for flag in FLAGS],
                f"-DPAPERCOLOR_DIAGNOSTIC_PURPLE_WHITE_MODE={mode}",
                "-I" + str(ROOT / "main"), "-I" + str(ROOT / "tools/color_lab"),
                str(implementation), str(ROOT / "main/display/papercolor_lut.cpp"),
                str(ROOT / "tools/color_lab/probe_frame.cpp"), "-o", str(binary),
            ], check=True)

            def render():
                return subprocess.run(
                    [str(binary), str(ROOT / "artifacts/color_lut/nominal-5bit.lut")],
        ...
        for row, (patch_id, modes) in enumerate(ROWS):
            ...
            record = {"id": patch_id, "name": patch["name"], "source_rgb": patch["rgb"],
                      "modes": modes, "variants": {}}
            for variant, mode in enumerate(modes):
                ...
                count = Counter(frame[y * 400 + x]
                                for y in range(y0 + 1, y1 - 1)
                                for x in range(x0 + 1, x1 - 1))
                record["variants"]["ABC"[variant]] = {
                    "mode": mode, "crop_sha256": sha(crop),
                    "native_counts": {NAMES[code]: count[code] for code in COLORS},
                }
            if row == 0:
                assert len({crops[row, variant] for variant in range(3)}) == 3
            else:
                assert crops[row, 0] == crops[row, 1] == crops[row, 2]
            records.append(record)

        assert len(payload) == 39600
        packed = tmp / "purple-white-abc-v1.bin"
        ...
        subprocess.run([
            ...
            "-I" + str(ROOT / "main"),
            str(ROOT / "tools/color_lab/probe_purple_white_abc_chart.cpp"),
            "-o", str(sampler),
        ], check=True)
    ...
    for index, code in enumerate([0, 1, 2, 3, 5, 6]):
        for y in range(52, 72):
            for x in range(12 + 64 * index, 68 + 64 * index):
                expected[y * 400 + x] = code
    for row in range(6):
    ...
    draw.text((12, 8), "PURPLE WHITE A/B/C", font=big, fill="black")
    draw.text((12, 30), "A: NOW   B: HALF WHITE   C: NONE", font=font, fill="black")
    for index, name in enumerate(["BLACK", "WHITE", "YELLOW", "RED", "BLUE", "GREEN"]):
        draw.text((12 + index * 64, 42), name, font=font, fill="black")
        draw.rectangle((11 + index * 64, 51, 68 + index * 64, 72), outline="black")
    for column, name in enumerate("ABCA"):
        draw.text((52 + 96 * column, 84), name, font=font, fill="black")
    for row, label in enumerate(LABELS):
        draw.text((12, 103 + 74 * row), label, font=font, fill="black")
    draw.text((12, 552), "Only row 09 changes; controls repeat A", font=font, fill="black")
    draw.text((12, 566), "Compare hue, brightness, white dots and grain", font=font, fill="black")
    draw.text((12, 582), "Keep whole chart visible. Button A: home", font=font, fill="black")
    for x in (2, 392):
        for y in (2, 592):
            draw.rectangle((x, y, x + 5, y + 5), fill="black")

    destination = ROOT / "artifacts/color_calibration/purple-white-abc-v1"
    ...
    manifest = {
        ...
        "sampler_codes_sha256": sha(codes), "production_color_algorithm_changed": False,
    ...
    hashes["artifacts/color_calibration/purple-white-abc-v1.bin"] = sha(payload)
    ...
    for record in records:
        print(record["id"], {key: value["native_counts"]
