#!/usr/bin/env python3
"""Purple-compensation smooth transition A/B against IMG_9944; nominal, not physical QA."""
import csv
import hashlib
import io
import json
...
def main():
    ...
    report = {'baseline':'IMG_9944 warm bypass','candidate':'purple source compensation smooth transition',
    ...
    frames, traces = {}, {}
    with tempfile.TemporaryDirectory(prefix='papercolor-purple-smooth-') as directory:
        tmp = Path(directory)
        ...
        for enabled in (False,True):
            ...
            flags += ['-DCONFIG_PAPERCOLOR_PURPLE_COMPENSATION_SMOOTH='+str(int(enabled))]
    ...
    eligible=(masks==43)&(5*rgb[:,0]>2*rgb[:,2])&(8*rgb[:,0]<5*rgb[:,2])
    legacy=(masks==43)&(rgb[:,2]>rgb[:,0])&(2*rgb[:,0]>=rgb[:,2])
    changed=np.any(previous!=current,axis=1)
    assert not np.any(changed & ~eligible)
    assert base['frame_sha256']==new['frame_sha256'], 'Entire canonical frame must stay identical'
    assert base['patches']==new['patches']
    ...
    for step,valid in ((2048,(index>>11)<31),(32,((index>>5)&63)<63),(1,(index&31)<31)):
        ...
        boundary.extend((legacy[a]!=legacy[b]) & (masks[a]==masks[b]))
    ...
    PROTECTED_RAMPS={'purple_light_dark','blue_black','blue_cyan','red_white','neutral','magenta_white','cyan_white','magenta_black','cyan_black','edge_white','edge_black','edge_neutral_red','edge_green',*[k for k in ramps if k.startswith('orange_')]}
    ...
    report['checks'] = {'targets_scanned':len(traces['candidate']),'continuity':continuity,
        'protected_uniform_byte_identical':len(controls),
    ...
    dest = ROOT/'artifacts/color_calibration/purple-smooth-comparison.json'
